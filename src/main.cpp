...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // maryjanecoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // maryjanecoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
if (pindexBest->nHeight == 1) { nSubsidy = 50000000 * COIN; }
...
// maryjanecoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes MaryJaneCoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // maryjanecoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // maryjanecoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // maryjanecoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // maryjanecoin: track money supply and mint amount info
...
// maryjanecoin: total coin age spent in transaction, in the unit of coin-days.
...
// maryjanecoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // maryjanecoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // maryjanecoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // maryjanecoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // maryjanecoin: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if(pindexBest != NULL && pindexBest->nHeight > 1)
        nCoinbaseMaturity = 100; //coinbase maturity  to 100 blocks
    // Size limits
...
bool CBlock::AcceptBlock()
{
    ...
    // maryjanecoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // maryjanecoin: check proof-of-stake
    ...
    // maryjanecoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // maryjanecoin: getblocks may not obtain the ancestor block rejected
    ...
    // maryjanecoin: if responsible for sync-checkpoint send it
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "MaryJaneCoin";
        CTransaction txNew;
        txNew.nTime = 1411912711;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nTime    = 1411912711;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1584262;
        if(fTestNet)
        {
            block.nNonce   = 0;
        }
    if (false  && (block.GetHash() != hashGenesisBlock)) {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("12c621ccfee4e004aad6d8319d156cbeb9110f4ba0d185b90d73a921125ea8ae"));
        block.print();
        ...
        // maryjanecoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x72, 0x32, 0x22, 0x02 };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // maryjanecoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // maryjanecoin: tell downloading node about the latest block if it's
